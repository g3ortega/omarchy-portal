...
assert.equal(context.dismissedServiceError, '', 'dismissing feedback preserves a distinct unseen backend error')
context.dismissNotice()
assert.equal(context.dismissedServiceError, 'status failed', 'dismissing the now-visible backend error suppresses it')
context.dismissedServiceError = ''
context.showMoment('status failed', true)
context.dismissNotice()
assert.equal(context.dismissedServiceError, 'status failed', 'identical visible feedback and backend errors need one dismissal')

...
// Classification of a listening port into "what is actually running there".
//
// Pure functions over the evidence scan-ports.sh gathers. No I/O, no QML types
// — so this file is unit-testable under plain node (see scripts/test-detect.mjs)
// and cheap to call on every refresh.
//
// Adding a framework is one entry in RULES. Rules are evaluated in order and
// the first match wins, so list specific rules before general ones.

// Rules name an icon; lib/Icons.js maps that name to a codepoint. Keeping the
// glyphs out of this file is what lets it run under plain node for tests, and
// means no raw Nerd Font characters live in logic code.
  ...
  for (var i = 0; i < RULES.length; i++) {
    if (RULES[i].test(e)) {
      return {
        ...
        label: RULES[i].label,
        icon: RULES[i].icon,
        color: BRAND[RULES[i].icon] || "",
        category: RULES[i].cat,
        web: RULES[i].cat === "dev" || has(WEB_PORTS, e.port)
      }
    }
  }
  return {
...
function decorate(entry) {
  ...
  return {
    port: entry.port,
    pid: entry.pid,
    start: entry.start === undefined ? null : entry.start,   // kernel start time: with the pid, one process
    comm: entry.comm || "",
